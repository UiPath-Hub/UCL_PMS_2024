	...
	submitData:async ()=>{
		if(appsmith.store["EditCatalog"]=== undefined)return await SP_INSERT_PMS_PRODUCT_CATALOG.run().then(()=>{ 
			if(SP_INSERT_PMS_PRODUCT_CATALOG.data !== undefined && SP_INSERT_PMS_PRODUCT_CATALOG.data.length !== 0){
				if(SP_INSERT_PMS_PRODUCT_CATALOG.data[0].RESULT_CODE === "DONE"){
					navigateTo('Product Catalog Dashboard', {}, 'SAME_WINDOW')
					removeValue("EditCatalog")
				}
			...
			if(SP_UPDATE_PMS_PRODUCT_CATALOG.data !== undefined && SP_UPDATE_PMS_PRODUCT_CATALOG.data.length !== 0){
				if(SP_UPDATE_PMS_PRODUCT_CATALOG.data[0].RESULT_CODE === "DONE"){
					navigateTo('Product Catalog Dashboard', {}, 'SAME_WINDOW');
					removeValue("EditCatalog");
				}
